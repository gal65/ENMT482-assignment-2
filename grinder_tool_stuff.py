...
##GM_T_TAB = transform_rotx(90, D_TAB)
##GM_T_TAB = np.matmul(GM_T_TAB, transform_rotx(90, [0, 0, 0])) # THIS IS A GUESS
##GM_T_TAB = np.matmul(GM_T_TAB, transform_roty(90, [0, 0, 0])) # THIS IS A GUESS
GM_T_TAB = ultimate_rotation(200,90,0,D_TAB)
GM_T_TAB = np.matmul(GM_T_TAB, transform_rotz(8, [0, 0, 0]))

# Approach offset
TAB_T_offset = offset(-70, -60, 30)
UR_T_TAB = np.matmul(UR_T_GM, GM_T_TAB)
UR_T_TAB_offset = np.matmul(UR_T_TAB, TAB_T_offset)

# Pull tab

# Final matrix calcs
T_TAB_np = np.matmul(UR_T_TAB_offset, gt_pull)
T_TAB_PULL1_np = np.matmul(UR_T_TAB, gt_pull)


# Convert np matrices to RDK matrices
T_BUT1 = rdk.Mat(T_BUT1_np.tolist())
T_BUT1_ON = rdk.Mat(T_BUT1_ON_np.tolist())
T_TAB = rdk.Mat(T_TAB_np.tolist())
...
J_int_over_cup = [83.570000, -51.430000, 48.210000, 0.000000, -16.620000, -30.090000]
J_int_BUT1 = [98.750420, -39.983399, 65.053132, -34.579756, -18.952260, -31.477093]
J_int_TAB = [-61.070000, -106.070000, -96.430000, -154.290000, 260.360000, -132.000000]#[-86.790000, -118.930000, -48.210000, -183.210000, 173.570000, 48.210000-90]
...
robot.MoveJ(J_int_BUT1, blocking=True)
...
robot.MoveL(T_TAB, blocking=True)
sleep(2)
